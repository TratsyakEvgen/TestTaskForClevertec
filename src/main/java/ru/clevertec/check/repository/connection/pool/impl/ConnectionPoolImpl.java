package ru.clevertec.check.repository.connection.pool.impl;
...
public class ConnectionPoolImpl implements ConnectionPool {
    ...
    @Override
    public void initPoolData(PoolData poolData) {
        int poolSize = poolData.getPoolSize();

        try {
            givenAwayConQueue = new ArrayBlockingQueue<>(poolSize);
            connectionQueue = new ArrayBlockingQueue<>(poolSize);
            for (int i = 0; i < poolSize; i++) {
                Connection connection = DriverManager.getConnection(poolData.getUrl(), poolData.getUser(), poolData.getPassword());
                PooledConnection pooledConnection = new PooledConnection(connection);
                connectionQueue.add(pooledConnection);
            }
        } catch (SQLException e) {
            throw new ConnectionPoolException("Error init ConnectionPool", e);
        }
    }
